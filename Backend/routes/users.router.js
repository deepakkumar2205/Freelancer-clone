...
const API = "https://freelancers-deepak.netlify.app";

router.post('/signup',express.json(),async function(request, response){
    const { email,who, emailVerified, password, userName } = request.body;
  ...
  if(userFromDB == null){
    ...
    if(userFromDB.emailVerified == 'yes'){    
        ...
        //!This if condition is used to validate the password.
            if(isPasswordCheck){
              const token = jwt.sign({ userId: myobjectId ,roleId:userFromDB.roleId,email:userFromDB.email,name:userFromDB.userName }, process.env.SECRET_KEY);
              // const tokenRole = jwt.sign({roll:userFromDB.roleId},process.env.SECRET_KEY)
              response.send({
      ...
      const mail =await sendEmail(email,"verification token" , `${API}/emailverify/${token}`)  
      response.status(406).send({message:"Invalid credentials"})
    }
  }
})
